...
/**
 * Ordena do processo mais novo para o mais antigo.
 *
 * A ordem é a do próprio número CNJ: primeiro o ano de ajuizamento, depois o
 * sequencial daquele ano. É como o advogado procura — o que entrou por último
 * está no alto — e independe da data em que o processo foi cadastrado aqui.
 */
export function ordenarProcessos(lista) {
  const chave = (p) => {
    const d = cnjDigitos(p.numeroCNJ);
    return d.length === 20
      ? `${d.slice(9, 13)}${d.slice(0, 7)}`
      : `0000${String(p.criadoEm || '').slice(0, 7).replace(/\D/g, '').padStart(7, '0')}`;
  };
  return [...lista].sort((a, b) => chave(b).localeCompare(chave(a)));
}
